...
export default class CacaoConnectionLabelUtil {
	...
	public static defineLabelPosition(
		...
		const waypoints: any = connection.waypoints;

		if (!waypoints && waypoints.length > 1) {
			return;
		}

		const labelBounds = (label as any).getBBox();
		labelBounds.x -= 5;
		labelBounds.y -= 5;
		...
		switch (preferredPosition) {
			case 'begin':
				...
				orientation = CacaoConnectionLabelUtil.getOrientation(waypoints[0], waypoints[1]);
				break;
			case 'end':
	 ...
	 */
	private static getOrientation(first: Point, second: Point): Orientation | undefined {
		if (first.x == second.x) {
			if (first.y < second.y) {
				return 'bottom';
			} else {
