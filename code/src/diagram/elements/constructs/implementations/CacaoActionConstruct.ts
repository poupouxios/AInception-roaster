...
export default class CacaoActionConstruct extends CacaoElement {
	...
	override drawConstruct(
		...
		if (CacaoUtils.isDefined(shape.collapsed) && !shape.collapsed) {
			...
			[agentTargetsShapeProps, agentTargetsHeight] = this.getAgentTargetsShapeProps(
				visualHeight + 10,
				shape,
				actionStep,
				playbookHandler,
	...
	getCommandsShapeProps(
		...
		const command = (y: number, command: Partial<CommandData>): [DrawProps[], number] => {
			const propsList: DrawProps[] = [];
			let yIndex = y;
		...
		return [[background(top, posY - top), title, ...commandPropsList], posY - top];
	}
	...
	getAgentTargetsShapeProps(
		...
		const agentTarget = (y: number, agentTarget: AgentTarget): [DrawProps[], number] => {
			const propsList: DrawProps[] = [];
			let yIndex = y;
			const horizontalMargin = 5;
			const horizontalPadding = 10;
		...
		if (CacaoUtils.isDefined(workflowStep.agent)) {
			...
			if (agent == undefined)
				throw Error('a step refer to an agent that is not existing in the playbook');
		...
		if (workflowStep.targets?.length != 0) {
			...
			for (const targetId of workflowStep.targets) {
				...
				if (target == undefined)
					throw Error('a step refer to a target that is not existing in the playbook');
