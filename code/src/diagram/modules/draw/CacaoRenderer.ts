...
export default class CacaoRenderer extends BaseRenderer {
	...
	constructor(eventBus: EventBus, canvas: Canvas, playbookHandler: PlaybookHandler) {
		super(eventBus, 1000);
 ...
 */
CacaoRenderer.prototype.canRender = (element: any): boolean => {
	let type: any;
	let returnValue = false;
	try {
		type = CacaoUtils.getTypeOfElement(element);
		returnValue = CacaoUtils.isConnectionType(type) || CacaoUtils.isConstructType(type);
	} catch (error) {}
	return returnValue;
};
 ...
 */
CacaoRenderer.prototype.drawShape = function (visuals: SVGElement, shape: Shape) {
	...
	const workflowStep = this._playbookHandler.getStep(cacaoElement.id);

	const elmt = cacaoElement.drawConstruct(visuals, shape, workflowStep, this._playbookHandler);
	return elmt;
};
 ...
 * @return the created SVGElement
 */
CacaoRenderer.prototype.drawConnection = function (visuals: SVGElement, connection: Connection) {
	...
	const elmt = cacaoElement.drawConnection(
