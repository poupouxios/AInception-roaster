...
export default class ConnectPreview {
	...
	bindEvents() {
		const connectionPreview: any = this._injector.get('connectionPreview', false);
		const canvas = this._canvas;

		this._eventBus.on('cacaoConnect.hover', LOW_PRIORITY, (event: any) => {
		...
		this._eventBus.on('cacaoConnect.move', (event: any) => {
			var context = event.context,
		...
		this._eventBus.on('cacaoConnect.cleanup', (event: any) => {
