...
import type { Rect } from 'diagram-js/lib/layout/LayoutUtil';
...
export default class CacaoLayouter extends BaseLayouter {
	...
	layoutConnection(connection: Connection, hints: LayoutConnectionHints = {}): Point[] {
		const source: Rect = hints.source || connection.source;
		const target: Rect = hints.target || connection.target;
		const waypoints: Point | Point[] = connection.waypoints;
