...
import type CacaoLayouter from './CacaoLayouter';
...
export default class CacaoModeling extends BaseModeling {
	...
	override createShape(
		...
		if (!shape.modelAssociated) {
			this._playbookHandler.createStep(cacaoConstruct.properties.modelType, shape.id);
	...
	updateGraphicalShapeOnCanvas(shape: Shape) {
		this._graphicsFactory.update('shape', shape, this._elementRegistry.getGraphics(shape));
	}
	 ...
	 * @param hints
	 */
	override removeConnection(connection: Connection, hints?: ModelingHints | undefined): void {
		if (connection?.source?.modelAssociated && connection?.target?.modelAssociated) {
	...
	updateConnectionType(connection: Connection, type: CacaoConnectionType) {
		if (!connection || !connection.type) {
			throw new Error('connection parameter must be defined, same for connection.type');
		}
