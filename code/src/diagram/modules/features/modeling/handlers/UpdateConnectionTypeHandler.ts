...
import { Connection, type ElementLike } from 'diagram-js/lib/model/Types';
...
export default class UpdateConnectionTypeHandler implements CommandHandler {
	...
	execute(context: any): ElementLike[] {
		...
		const newType = context.type;
		const oldType = CacaoUtils.getTypeOfElement(connection);
