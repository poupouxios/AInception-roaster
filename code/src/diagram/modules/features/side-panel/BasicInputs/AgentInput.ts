...
export class AgentInput extends BasicInput {
	...
	addToContainer(): void {
		...
		this._agentButton = new PanelButton(this._displayedValue, this._container, () => {
			this.showPanel();
		});
		...
		this._agentTargetPanel.addButton('Confirm', () => {
			const submitedValue = this._agentTargetPanel.submit();
			this._tempValue =
	...
	private changeDisplayedValue(submitedValue: string) {
		if (Object.keys(valueToDisplay).includes(this._propertyType)) {
			const agent = this._playbookHandler.getAgent(submitedValue)
