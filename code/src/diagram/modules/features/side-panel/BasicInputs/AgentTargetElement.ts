...
export class AgentTargetElement extends BasicInput {
	...
	constructor(
		...
		[[this._key, this._val]] = Object.entries(this._agentTarget) as [[string, any]];
		this._boxConstructor = this._isAgent ? RadioBoxInput : CheckboxInput;
	}

	addToContainer(): void {
		const boxContainer = document.createElement('td');
		...
		const type = this._val.type ? this._val.type : '';
		typeContainer.textContent = type;

		const nameContainer = document.createElement('td');
		const name = this._val.name ? this._val.name : '';
		nameContainer.textContent = name;

		const descContainer = document.createElement('td');
		const desc = this._val.description ? this._val.description : '';
		...
		agentPanel.addButton('Confirm', () => {
			const agentValue = agentPanel.confirm();
			const typeValue = agentValue.type;
		...
		const removeButton = new PanelButton('x', editButtonContainer, () => {
