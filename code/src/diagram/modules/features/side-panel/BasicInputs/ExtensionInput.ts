...
import { extractSchemaTypes, schemaDictWithoutCommands } from '../../../model/SchemaTypes';
...
export class ExtensionInput extends BasicInput {
	...
	constructor(
		...
		if (Object.keys(initialValue).length === 0) {
			...
			[[this._key, this._value]] = Object.entries(initialValue) as [[string, string]];
			try {
				if (this._key != '' && typeof this._value !== 'string') {
					const stringified = JSON.stringify(this._value, null, 2);
	...
	loadProps() {
		...
		if (
			this._key != '' &&
			(this._playbookHandler.playbook['extension_definitions'] as any)[this._key]
		) {
			...
			const schemaField = (this._playbookHandler.playbook['extension_definitions'] as any)[
				this._key
			]['schema'];
			var schema: any;
			if (schemaField) {
					...
					try {
						fetch(schemaField)
							...
							.then(data => {
								...
								props = extractSchemaTypes(data as any, schemaDictWithoutCommands);
			...
			try {
				const schemaData = extractSchemaTypes(schema, schemaDictWithoutCommands);
				props = schemaData;
				this._usesSchema = true;
			} catch (error) {
				props = {
	...
	addToContainer(): void {
		...
		this._opendropdown = new DropDownInput(this._inputName, this._key, this._options);
		this._opendropdown.setContainer(this._container);
		const editButton = new PanelButton('Edit', this._container, () => {
	...
	submit(): any {
		if (this._usesSchema) {
			return [[this._opendropdown.submit() as string], this._propertyPanel?.confirm()];
		}
		const temp = this._propertyPanel?.confirm()['extension'];
		let parsed = temp;
		try {
