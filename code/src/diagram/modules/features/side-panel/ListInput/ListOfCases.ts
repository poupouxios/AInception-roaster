...
export class ListOfCases extends ComplexInput {
	...
	constructor(propertyName: string, propertyType: string, container: HTMLElement) {
	...
	addToContainer(): void {
		...
		labelTag = labelHeader.firstChild ? (labelHeader.firstChild as HTMLElement) : null;
		while (labelTag && labelTag.tagName != 'LABEL') {
			labelTag = labelHeader.firstChild ? (labelTag.nextSibling as HTMLElement) : null;
		}
	...
	addElement(item: Array<string>) {
		const itemInput = this.createBasicInput(this._propertyName + this._elements.length, item);
