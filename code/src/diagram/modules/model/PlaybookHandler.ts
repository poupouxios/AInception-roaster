...
import {
	ExecutionStatus,
	type StatusElement,
} from '../../modules/model/status/status-model/ExecutionStatus';
...
export default class PlaybookHandler {
	...
	getShapeStatus(stepId: string) {
		this._executionStatus.step_results;
		for (const key in this._executionStatus.step_results) {
			if (key === stepId) {
				return this._executionStatus.step_results[key].status;
			}
		}
		return undefined;
	}
	...
	hasExecutionStatus() {
		...
		if (
			...
			Object.keys(this._executionStatus.step_results).length === 0
		) {
			return false;
		}
		return true;
